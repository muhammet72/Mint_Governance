...
const moveBlocks = require("../utils/moveBlocks").default;
...
describe("MyGovernor", function () {
  async function deployFixture() {
    ...
    const governor = await MyGovernor.deploy(futureAddress, 10, 20);
  ...
  describe("after proposing", () => {
    ...
    describe("after voting", () => {
      async function afterVotingFixture() {
        ...
        // wait for the votingDelay blocks voting delay
        let votingDelay = await governor.votingDelay();
        await moveBlocks(votingDelay.toNumber() + 1);
      ...
      it("should allow executing the proposal", async () => {
        const { governor, proposalId, token, owner } = await loadFixture(
          afterVotingFixture
        );

        // wait for the votingPeriod to pass...
